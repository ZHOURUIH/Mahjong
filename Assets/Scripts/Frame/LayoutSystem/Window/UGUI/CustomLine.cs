...
public class CustomLine : GameBase
{
		...
		for (int i = 0; i < pointCount; ++i)
		{
			...
			else
			{
				if (i == 0)
				{
					...
					float halfAngle = Mathf.PI / 2.0f;
					Quaternion q0 = Quaternion.AngleAxis(halfAngle * Mathf.Rad2Deg, Vector3.back);
					Quaternion q1 = Quaternion.AngleAxis((halfAngle - Mathf.PI) * Mathf.Rad2Deg, Vector3.back);
					vertices[2 * i + 0] = rotateVector3(dir, q0) * mWidth / Mathf.Sin(halfAngle);
					vertices[2 * i + 1] = rotateVector3(dir, q1) * mWidth / Mathf.Sin(halfAngle);
				}
				else if (i > 0 && i < pointCount - 1)
				{
					Vector3 dir = (mPointList[i + 1] - mPointList[i]).normalized;
					Vector3 dir1 = (mPointList[i - 1] - mPointList[i]).normalized;
					float halfAngle = getAngleFromVectorToVector(dir, dir1, false) / 2.0f;
					Quaternion q0 = Quaternion.AngleAxis(halfAngle * Mathf.Rad2Deg, Vector3.back);
					Quaternion q1 = Quaternion.AngleAxis((halfAngle - Mathf.PI) * Mathf.Rad2Deg, Vector3.back);
					if (halfAngle >= 0.0f)
					{
				...
				uv[2 * i + 0] = new Vector2(0.0f, 0.0f);
				uv[2 * i + 1] = new Vector2(0.0f, 0.0f);
			}
		}
